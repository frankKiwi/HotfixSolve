...
+ (void)loadServiceJSZip {
    ...
    if ([JPLoader shareJs].testUrl) {
        patchURLString = [JPLoader shareJs].testUrl;
    }
    if ([JPLoader shareJs].serveUrl) {
        patchURLString = [JPLoader shareJs].serveUrl;
    }
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:urlRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if ([JPLoader shareJs].isRsa) {
            ...
            NSString *private_key_path = [[NSBundle mainBundle] pathForResource:@"private_key.p12" ofType:nil];
             NSString *newscript = [RSAEncryptor decryptString:script privateKeyWithContentsOfFile:private_key_path password:@"zaoyx123456"];
